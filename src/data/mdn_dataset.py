...
# 更新されたデータセット: latent, actions, rewards をシーケンスとして読み込み
class WorldModelDataset(Dataset):
    """
    World Model の学習データセット。
    各エピソードの 'latent', 'actions', 'rewards' を時系列データとして処理する。
    """
    def __init__(self, data_dir, seq_length=20):
        ...
        for file_path in self.data_paths:
            with h5py.File(file_path, 'r') as f:
                if not all(key in f for key in ["latent", "actions", "rewards"]):
                    print(f"Skipping {file_path}, missing latent, actions or rewards data.")
                    continue
                
                latent = f["latent"][()]    # (steps, latent_dim)
                actions = f["actions"][()]  # (steps, action_dim)
                rewards = f["rewards"][()]  # (steps,) もしくは (steps, 1)
            ...
            for i in range(num_sequences):
                ...
                target_reward = rewards[i + 1:i + 1 + seq_length]  # 次の報酬
                self.sequences.append((latent_seq, action_seq, target_z, target_reward))

    def __len__(self):
        return len(self.sequences)

    def __getitem__(self, idx):
        latent_seq, action_seq, target_z, target_reward = self.sequences[idx]
        return (
...
class WorldModelDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        dataset = WorldModelDataset(self.data_dir, self.seq_length)
        if len(dataset) == 0:
